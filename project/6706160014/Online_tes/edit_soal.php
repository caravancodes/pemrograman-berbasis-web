...
$ujian_exe = mysqli_query("select nama_ujian,id_mp from ujian where id_ujian='".$id_ujian."'");
while($data = mysqli_fetch_assoc($ujian_exe)){
	$id_mp = $data['id_mp'];
	$nama_ujian = $data['nama_ujian'];	
	}
$soal_exe = mysqli_query("select * from soal where id_ujian='".$id_ujian."' order by id_soal ");
echo "<div class='row-fluid' id='div_edit_soal'>
		<div class='block'>
			<div class='navbar navbar-inner block-header'>
				<div class='muted pull-left'>Edit Soal</div>
			</div>";
if(mysqli_num_rows($soal_exe) > 0){
	...
	$sql_jawaban_exe = mysqli_query($sql_jawaban);
	$pil_jawaban = array();
	while($d_jawaban = mysqli_fetch_assoc($sql_jawaban_exe)){
	...
	echo "</div>";
	while($data = mysqli_fetch_assoc($soal_exe)){
?>		
	<div class='soal_edit'>
