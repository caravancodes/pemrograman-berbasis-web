...
$pass_lama_db = mysqli_result(mysqli_query("select password from user where id_user='".$id_user."'"),0);
if($pass_lama_db == $pass_lama){
	$update_pass = mysqli_query("update user set password='".$pass_baru."' where id_user='".$id_user."'");
	if($update_pass){
	$status = 1;
	}
	}
