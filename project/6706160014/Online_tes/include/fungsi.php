...
// $field = new Array();
for ($j = 0; $j < mysqli_num_fields($hasil); ++$j) {
// $table = mysqli_field_table($hasil, $i);
$field[$j] = mysqli_field_name($hasil, $j);
//printf(" $table "." : "." $field ");
$data.= "<tr><td>
...
$hasil_exe=mysqli_query($query);
if($hasil_exe){
if(mysqli_num_rows($hasil_exe) != 0) {
$hasil='';
$hasil_perbaris='';
$j=1;
while($data=mysqli_fetch_array($hasil_exe)){
    if($link_ke_edit ==""){
...
function tampilkan_tabel($sql,$tampil,$pesan,$judul=""){
	$sql_exc = mysqli_query($sql);
	//query sukses
	if($sql_exc){
		$jml_baris =mysqli_num_rows($sql_exc);
		if($jml_baris > 0){
			...
			$hasil="<table class='listing' cellspacing='0' cellpadding='1' align='center'>
				...
				<tr>
				<th>No</th>";
			//$jum_kolom =mysqli_num_fields($sql_exc);
			for($i = 0; $i < count($tampil); $i++){
			$hasil.= "<th>$tampil[$i]</th>";
				}
			$hasil.= "</thead><tbody>";
			$h = 1;
			while($baris =mysqli_fetch_array($sql_exc)){
				$hasil.= "<tr><td>".$h."</td>";
