...
$_SESSION['level_kj'] == 'guru';
if($_SESSION['level_kj'] == 'guru'){
$mp_exe = mysqli_query("select mapel.id_mp as id_mp,nama_mp,mengajar.id_kelas as id_kelas,nama_kelas from mapel,mengajar,kelas where mapel.id_mp = mengajar.id_mp and kelas.id_kelas=mengajar.id_kelas and id_guru='".$_SESSION['nis_kj']."'");
}
else if($_SESSION['level_kj'] == 'admin') {
	$mp_exe = mysqli_query("select mapel.id_mp as id_mp,nama_mp,mengajar.id_kelas as id_kelas,nama_kelas from mapel,mengajar,kelas where mapel.id_mp = mengajar.id_mp and kelas.id_kelas=mengajar.id_kelas");
	}
$jml_mp = mysqli_num_rows($mp_exe);
if($jml_mp > 0){
?>
		<?php
		while($data = mysqli_fetch_assoc($mp_exe)){
			$jml_ujian = mysqli_result(mysqli_query("select count(*) from ujian where id_mp='".$data['id_mp']."' and id_kelas='".$data['id_kelas']."'"),0);
