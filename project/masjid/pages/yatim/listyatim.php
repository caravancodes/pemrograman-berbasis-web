...
<div id="post">
    <h1 class="title">:: Data Anak Yatim ::</h1>
    <div class="entry">   

    <div align="center" class="link">
        <a href=?act=InputYatim&action=TAMBAH>| Input Data |</a>
        <a href=?act=ListYatim>| List Data |</a>
    </div>   
	<br/>
    ...
    <?php
        ...
        if(mysql_num_rows($hasil)!=0) {            
            echo "<table width='1000px' border='0' align='center' cellpadding='0' cellspacing='0' class='tbl_form'>
                    <tr class='head'>
                                 ...
                                 <td width='200px'><div align='center'><font size='2' face='Verdana'><strong>Nama Anak Yatim</strong></font></div></td>
			        ...
			        <td>
                                        <center>
                                        <a href=?act=InputYatim&action=UBAH&id=$baris[0]>[edit]</a>"; ?>
                                        <a href="?act=ListYatim&action=HAPUS&id=<?php print $baris[0] ?>" onClick="if (!confirm('Anda yakin menghapus data anak yatim <?php print $baris[1]?>?')) return false;">[hapus]</a>
                            <?php
                           echo "       </center>
                                </td>								
                                 <td>$baris[1]</td>
        ...
        } else {echo "<b>Data anak yatim masih kosong !</b>";}

    ?>

    <?php
       if ($_GET['action']=="HAPUS") {
            Hapus(" yatim "," id ='".$_GET['id']."' ","?act=ListYatim");
       }
    ?>
    </div>
       ...
       <?php
		...
		$hasil1=bukaquery("select id, nama, jk, pendidikan, tmp_lahir, tgl_lahir, alamat
                ...
                $i=$jumlah1/19;
                $i=ceil($i);
                echo(" Data : $jumlah<a target=_blank href=http://".host()."/masjid/pages/yatim/laporanyatim.php?&keyword=$keyword>| Laporan |</a> | Halaman: ");
                for($j=1;$j<=$i;$j++){
                    $awal=(($j-1)*19+$j)-1;
                    echo("<a href='?act=ListYatim&awal=$awal&page=$j'>[$j]</a> ");
                }
             }
       ?>
    </div>
