...
<div id="post">
    <h1 class="title">:: Data Pemasukan ::</h1>
    <div class="entry">   
    ...
    <?php
        ...
        if(mysql_num_rows($hasil)!=0) {            
            echo "<table width='900px' border='0' align='center' cellpadding='0' cellspacing='0' class='tbl_form'>
			        ...
			        <td>
                                        <center>
                                        <a href=?act=InputPemasukan&action=UBAH&id=$baris[0]>[edit]</a>"; ?>
                                        <a href="?act=ListPemasukan&action=HAPUS&id=<?php print $baris[0] ?>" onClick="if (!confirm('Anda yakin menghapus data zakat/sodaqoh/infaq/- <?php print $baris[1]?>?')) return false;">[hapus]</a>
                            <?php
                           echo "       </center>
                                </td>								
                                 <td>$baris[1]</td>
    ...
    <?php
       if ($_GET['action']=="HAPUS") {
            Hapus(" pemasukan "," id ='".$_GET['id']."' ","?act=ListPemasukan");
       }
       $BtnPrint=$_POST['BtnPrint'];
       if (isset($BtnPrint)) {
           echo"<html><head><title></title><meta http-equiv='refresh' content='2;pages/pemasukan/laporanpemasukan.php?&keyword=$keyword'></head><body></body></html>";
    ...
    ?>
       ...
       <?php
            if(mysql_num_rows($hasil)!=0) {
		$hasil1=bukaquery("select id, pemberi, besarnya, jenis, tanggal from pemasukan
                 where
                 pemberi like '%".$keyword."%' or
                 besarnya like '%".$keyword."%' or
                 tanggal like '%".$keyword."%' or
                ...
                $i=$jumlah1/19;
                $i=ceil($i);
                echo(" Data : $jumlah<a target=_blank href=http://".host()."/masjid/pages/pemasukan/laporanpemasukan.php?&keyword=$keyword>| Laporan |</a> | Halaman: ");
                for($j=1;$j<=$i;$j++){
                    $awal=(($j-1)*19+$j)-1;
                    echo("<a href='?act=ListPemasukan&awal=$awal&page=$j'>[$j]</a> ");
                }
             }
       ?>
    </div>
