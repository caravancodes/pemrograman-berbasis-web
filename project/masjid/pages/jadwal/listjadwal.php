...
<div id="post">
    <h1 class="title">:: Data Jadwal Kegiatan ::</h1>
    <div class="entry">   
    ...
    <?php
        ...
        $_sql = "select id, hari, jam, jenis from jadwal
                 ...
                 jam like '%".$keyword."%' or
                 jenis like '%".$keyword."%' 
                 order by hari ASC LIMIT $awal,20";
        $hasil=bukaquery($_sql);
        $jumlah=mysql_num_rows($hasil);

        if(mysql_num_rows($hasil)!=0) {            
            echo "<table width='700px' border='0' align='center' cellpadding='0' cellspacing='0' class='tbl_form'>
                    <tr class='head'>
			        ...
			        <td>
                                        <center>
                                        <a href=?act=InputJadwal&action=UBAH&id=$baris[0]>[edit]</a>"; ?>
                                        <a href="?act=ListJadwal&action=HAPUS&id=<?php print $baris[0] ?>" onClick="if (!confirm('Anda yakin menghapus data jadwal <?php print $baris[1]?>?')) return false;">[hapus]</a>
                            <?php
                           echo "       </center>
                                </td>								
                                 <td>$baris[1]</td>
        ...
        } else {echo "<b>Data jadwal kegiatan masih kosong !</b>";}

    ?>

    <?php
       if ($_GET['action']=="HAPUS") {
            Hapus(" jadwal "," id ='".$_GET['id']."' ","?act=ListJadwal");
       }
       $BtnPrint=$_POST['BtnPrint'];
       if (isset($BtnPrint)) {
           echo"<html><head><title></title><meta http-equiv='refresh' content='2;pages/jadwal/laporanjadwal.php?&keyword=$keyword'></head><body></body></html>";
    ...
    ?>
       ...
       <?php
            if(mysql_num_rows($hasil)!=0) {
		$hasil1=bukaquery("select id, hari, jam, jenis from jadwal
                 where
                 hari like '%".$keyword."%' or
                 jam like '%".$keyword."%' or
                 jenis like '%".$keyword."%'
                ...
                $i=$jumlah1/19;
                $i=ceil($i);
                echo(" Data : $jumlah<a target=_blank href=http://".host()."/masjid/pages/jadwal/laporanjadwal.php?&keyword=$keyword>| Laporan |</a> | Halaman: ");
                for($j=1;$j<=$i;$j++){
                    $awal=(($j-1)*19+$j)-1;
                    echo("<a href='?act=ListJadwal&awal=$awal&page=$j'>[$j]</a> ");
                }
             }
       ?>
    </div>
