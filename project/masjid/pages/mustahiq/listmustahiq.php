...
<div id="post">
    ...
    <?php
        ...
        if(mysql_num_rows($hasil)!=0) {            
            echo "<table width='1100px' border='0' align='center' cellpadding='0' cellspacing='0' class='tbl_form'>
                    <tr class='head'>
                                 ...
                                 <td width='200px'><div align='center'><font size='2' face='Verdana'><strong>Nama Mustahiq</strong></font></div></td>
			        ...
			        <td>
                                        <center>
                                        <a href=?act=InputMustahiq&action=UBAH&id=$baris[0]>[edit]</a>"; ?>
                                        <a href="?act=ListMustahiq&action=HAPUS&id=<?php print $baris[0] ?>" onClick="if (!confirm('Anda yakin menghapus data mustahiq <?php print $baris[1]?>?')) return false;">[hapus]</a>
                            <?php
                           echo "       </center>
                                </td>								
                                 <td>$baris[1]</td>
        ...
        } else {echo "<b>Data mustahiq masih kosong !</b>";}

    ?>

    <?php
       if ($_GET['action']=="HAPUS") {
            Hapus(" mustahiq "," id ='".$_GET['id']."' ","?act=ListMustahiq");
       }
    ?>
    </div>
       ...
       <?php
		...
		$hasil1=bukaquery("select id, nama, jk, pendidikan, tmp_lahir, tgl_lahir, alamat,pekerjaan
                ...
                $i=$jumlah1/19;
                $i=ceil($i);
                echo(" Data : $jumlah<a target=_blank href=http://".host()."/masjid/pages/mustahiq/laporanmustahiq.php?&keyword=$keyword>| Laporan |</a> | Halaman: ");
                for($j=1;$j<=$i;$j++){
                    $awal=(($j-1)*19+$j)-1;
                    echo("<a href='?act=ListMustahiq&awal=$awal&page=$j'>[$j]</a> ");
                }
             }
       ?>
    </div>
