...
<div id="post">
    ...
    <div class="entry">
            ...
            <?php
                ...
                if (isset($BtnSimpan)) {
                    ...
                    if ((!empty($id))&&(!empty($tgl))) {
                        switch($_GET['action']) {
                            case "TAMBAH":
                                Tambah(" tambahjaga "," '$tgl','$id'", " Tambahan Jaga " );
                ...
                if(mysql_num_rows($hasil)!=0) {
                    ...
                    while($baris = mysql_fetch_array($hasil)) {                        
                      echo "<tr class='isi'>
                                <td width='70'>
                                    <center>"; ?>
                                    <a href="?act=InputTambahJaga&action=HAPUS&tgl=<?php print $baris[0] ?>&id=<?php print $baris[1] ?>" onClick="if (!confirm('Anda yakin menghapus data tambahan jaga tanggal <?php print $baris[0]?>?')) return false;">[hapus]</a>
                            <?php
                            echo "</center>
                                </td>
                                <td>$baris[0]</td>
        ...
        if(mysql_num_rows($hasil)!=0) {
                ...
                for($j=1;$j<=$i;$j++){
                    $awal=(($j-1)*19+$j)-1;
                    echo("<a href='?act=InputTambahJaga&action=TAMBAH&id=$id&awal=$awal&page=$j'>[$j]</a>");
                }
        }
        ?>
    </div>
