...
<div id="post">
    ...
    <div class="entry">
        <form name="frm_aturadmin" onsubmit="return validasiIsi();" method="post" action="" enctype=multipart/form-data>
            ...
            <?php
                ...
                if (isset($BtnSimpan)) {
                    ...
                    if (!empty($tnj)) {
                        switch($_GET['action']) {
                            case "TAMBAH":
                                Tambah(" set_jgtambah  ","'$tnj'", " Set tambah jaga " );
                                echo"<meta http-equiv='refresh' content='1;URL=?act=InputSetTambahJaga&action=TAMBAH&tnj='$tnj'>";
                                break;
                        }
                    }else if (empty($tnj)){
            ...
            ?>
            <div style="width: 598px; height: 150px; overflow: auto; padding: 5px">
            <?php
                $_sql = "SELECT tnj from set_jgtambah  ORDER BY tnj ASC";
                ...
                if(mysql_num_rows($hasil)!=0) {
                    ...
                    while($baris = mysql_fetch_array($hasil)) {
                      echo "<tr class='isi'>
                                <td>
                                   <center>";?>
                                    <a href="?act=InputSetTambahJaga&action=HAPUS&tnj=<?php print $baris[0] ?>" onClick="if (!confirm('Anda yakin menghapus set tambah jaga ?')) return false;">[hapus]</a>
                            <?php
                            echo "</center>
                                </td>
        ...
        <?php
            if ($_GET['action']=="HAPUS") {
                Hapus(" set_jgtambah  "," tnj ='".$tnj."' ","?act=InputSetTambahJaga&action=TAMBAH&tnj=".$tnj);
            }
        ?>
    </div>
