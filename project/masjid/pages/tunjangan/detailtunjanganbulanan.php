...
<div class="y">
<div id="post">
    <h1 class="title">::[ Master Tunjangan Bulanan ]::</h1>
    ...
    <div class="entry">
        <form name="frm_aturadmin" onsubmit="return validasiIsi();" method="post" action="" enctype=multipart/form-data>
            ...
            <?php
                ...
                $_sql = "SELECT id,nama,tnj from master_tunjangan_bulanan ORDER BY nama ASC LIMIT $awal,20";
                    ...
                    while($baris = mysql_fetch_array($hasil)) {
                      echo "<tr class='isi'>
                                <td>
                                    ...
                                    <a href="?act=DetailTunjanganBulanan&action=HAPUS&id=<?php print $baris[0] ?>&nama=<?php print str_replace(" ","_",$baris[1]) ?>" onClick="if (!confirm('Anda yakin menghapus master tunjangan harian <?php print $baris[1]?>?')) return false;">[hapus]</a>
                            <?php
                            echo "</center>
                                </td>
                                <td>$baris[1]</td>
            ...
            } else {echo "<b>Data Master Tunjangan Bulanan !</b>";}
        ?>
        </div>
        </form>
        <?php
        ...
        if(mysql_num_rows($hasil)!=0) {
                $hasil1=bukaquery("SELECT nama,tnj from master_tunjangan_bulanan ORDER BY nama ");
                ...
                for($j=1;$j<=$i;$j++){
                    $awal=(($j-1)*19+$j)-1;
                    echo("<a href='?act=DetailTunjanganBulanan&action=TAMBAH&nama=$nama&awal=$awal&page=$j'>[$j]</a> ");
                }
        }
        ?>
    </div>
