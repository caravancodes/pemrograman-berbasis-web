    ...
    <?php
        ...
        if(mysql_num_rows($hasil)!=0) {            
            echo "<table width='1000px' border='0' align='center' cellpadding='0' cellspacing='0' class='tbl_form'>
			        ...
			        <td>
                                        <center>
                                        <a href=?act=InputJanda&action=UBAH&id=$baris[0]>[edit]</a>"; ?>
                                        <a href="?act=ListJanda&action=HAPUS&id=<?php print $baris[0] ?>" onClick="if (!confirm('Anda yakin menghapus data janda <?php print $baris[1]?>?')) return false;">[hapus]</a>
                            <?php
                           echo "       </center>
                                </td>								
                                 <td>$baris[1]</td>
        ...
        } else {echo "<b>Data janda masih kosong !</b>";}

    ?>

    <?php
       if ($_GET['action']=="HAPUS") {
            Hapus(" janda "," id ='".$_GET['id']."' ","?act=ListJanda");
       }
    ?>
    </div>
       ...
       <?php
            if(mysql_num_rows($hasil)!=0) {
		$hasil1=bukaquery("select id, nama, pendidikan, tmp_lahir, tgl_lahir, alamat
                 from janda
                 where
                 nama like '%".$keyword."%' or
                 pendidikan like '%".$keyword."%' or
                ...
                $i=$jumlah1/19;
                $i=ceil($i);
                echo(" Data : $jumlah<a target=_blank href=http://".host()."/masjid/pages/janda/laporanjanda.php?&keyword=$keyword>| Laporan |</a> | Halaman: ");
                for($j=1;$j<=$i;$j++){
                    $awal=(($j-1)*19+$j)-1;
                    echo("<a href='?act=ListJanda&awal=$awal&page=$j'>[$j]</a> ");
                }
             }
       ?>
    </div>
