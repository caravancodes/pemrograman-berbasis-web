...
<div id="post">
    <h1 class="title">:: Data Fakir Miskin ::</h1>
    <div class="entry">   

    <div align="center" class="link">
        <a href=?act=InputFakir&action=TAMBAH>| Input Data |</a>
        <a href=?act=ListFakir>| List Data |</a>
    </div>   
	<br/>
    ...
    <?php
        ...
        $_sql = "select id, nama, jk, pendidikan, tmp_lahir, tgl_lahir, alamat
                 from fakir
                 where
        ...
        if(mysql_num_rows($hasil)!=0) {            
            echo "<table width='1000px' border='0' align='center' cellpadding='0' cellspacing='0' class='tbl_form'>
                    <tr class='head'>
                                 ...
                                 <td width='200px'><div align='center'><font size='2' face='Verdana'><strong>Nama Fakir Miskin</strong></font></div></td>
			        ...
			        <td>
                                        <center>
                                        <a href=?act=InputFakir&action=UBAH&id=$baris[0]>[edit]</a>"; ?>
                                        <a href="?act=ListFakir&action=HAPUS&id=<?php print $baris[0] ?>" onClick="if (!confirm('Anda yakin menghapus data fakir miskin <?php print $baris[1]?>?')) return false;">[hapus]</a>
                            <?php
                           echo "       </center>
                                </td>								
                                 <td>$baris[1]</td>
        ...
        } else {echo "<b>Data fakir miskin masih kosong !</b>";}

    ?>

    <?php
       if ($_GET['action']=="HAPUS") {
            Hapus(" fakir "," id ='".$_GET['id']."' ","?act=ListFakir");
       }
    ?>
    </div>
       ...
       <?php
            if(mysql_num_rows($hasil)!=0) {
		$hasil1=bukaquery("select id, nama, jk, pendidikan, tmp_lahir, tgl_lahir, alamat
                 from fakir
                 where
                 nama like '%".$keyword."%' or
                 jk like '%".$keyword."%' or
                ...
                $i=$jumlah1/19;
                $i=ceil($i);
                echo(" Data : $jumlah<a target=_blank href=http://".host()."/masjid/pages/fakir/laporanfakir.php?&keyword=$keyword>| Laporan |</a> | Halaman: ");
                for($j=1;$j<=$i;$j++){
                    $awal=(($j-1)*19+$j)-1;
                    echo("<a href='?act=ListFakir&awal=$awal&page=$j'>[$j]</a> ");
                }
             }
       ?>
    </div>
