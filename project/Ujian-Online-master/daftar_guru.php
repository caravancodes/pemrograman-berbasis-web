...
<div class="span12">
	...
	$sql_exe_all = mysqli_query($sql_all);
	$jml_all = mysqli_num_rows($sql_exe_all);
	$jml_hal = round($jml_all / $perhal); // hitung jumlah seluruh halaman
	$sql_tampil="select * from guru limit ".$index_hal.",".$perhal;
	$sql_exe_tampil = mysqli_query($sql_tampil);
//	echo $sql_tampil;
	if(mysqli_num_rows($sql_exe_tampil) > 0){
			...
			<div class='span12'>                                    
				<table cellpadding='0' cellspacing='0' border='0' class='table table-striped table-bordered'>
					<thead>
						<tr>
							<th>no</th>";	
								$jum_kolom = mysqli_num_fields($sql_exe_tampil);
								for($i = 0; $i < $jum_kolom; $i++){
								echo "<th>".mysqli_field_name($sql_exe_tampil,$i)."</th>";
								}
							echo "<th>aksi</th>
						</tr>
					</thead>";
				...
				$no = $index_hal + 1;
				while($data_baris = mysqli_fetch_assoc($sql_exe_tampil)){
