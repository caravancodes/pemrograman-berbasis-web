...
$nama_mp = mysqli_result(mysqli_query("select nama_mp from mapel where id_mp='".$id_mp."'"),0);
?>
<div class="row-fluid">
<form id="f_ujian">
<input type="hidden" style="width: 200px" name="id_mp" id="id_mp" value="<?php echo $id_mp ?>" class="inputan input-block-level"/>	
<input type="hidden" style="width: 200px" name="id_kelas" id="id_kelas" value="<?php echo $id_kelas ?>" class="inputan input-block-level"/>	
<input type="text" style="width: 200px" name="nama_ujian" id="nama_ujian" value="Nama Ujian" class="inputan input-block-level" onfocus="bersihkan(this)" onblur="kembali_semula(this)"/>
<input type="text" style="width: 200px" name="tanggal" id="tanggal" value="Tanggal ujian" class="inputan input-block-level" onfocus="bersihkan(this)" onblur="kembali_semula(this)"/>
<input type="text" style="width: 200px" name="waktu" id="waktu" value="Waktu (dalam menit)" class="inputan input-block-level" onfocus="bersihkan(this)" onblur="kembali_semula(this)"/>
<input type="text" style="width: 200px" name="keterangan" id="keterangan" size="60" value="Keterangan" class="inputan input-block-level" onfocus="bersihkan(this)" onblur="kembali_semula(this)"/>
<br/><span class="btn btn-success tombol tambah" id="simpan_ujian" onclick="simpan_ujian(this)"><i class="icon-white icon-plus"></i> Tambah Ujian</span>
<span id="update_ujian" style="display:none">
<span class="btn btn-success tombol simpan" onclick="update_ujian(this)"><i class="icon-white icon-ok"></i> Ubah Ujian</span>
<span class="btn btn-warning tombol batal" onclick="batal_update()"><i class="icon-white icon-remove"></i> Batal</span>
</span>
</form>
<div class="block">
...
$no = 1;
while($data = mysqli_fetch_assoc($sql_exe)){
	...
	$jml_soal = mysqli_result(mysqli_query("select count(*) from soal where id_ujian='".$data['id_ujian']."'"),0);
	echo "<div class='jml_soal'>".$jml_soal."</div>";
	echo "<div class='ket'>".$data['keterangan']."</div>";
	echo "<div class='aksi'><span class='btn btn-success tombol edit' onclick='edit_ujian(this,\"".$data['id_ujian']."\")'><i class='icon-white icon-pencil'></i> Ujian</span>&nbsp;<span class='btn btn-success tombol tambah' onclick='tambah_soal_ujian(\"".$data['id_ujian']."\")'>";
	echo "<i class='icon-white icon-plus'></i> Soal</span>&nbsp;<span class='btn btn-success tombol edit' onclick='edit_soal_ujian(\"".$data['id_ujian']."\")'><i class='icon-white icon-pencil'></i> Soal</span>&nbsp;<span class='btn btn-danger tombol hapus' onclick='hapus_ujian(this,\"".$data['id_ujian']."\")'><i class='icon-white icon-trash'></i> Hapus</span></div>";
	echo "</div>";
	}
