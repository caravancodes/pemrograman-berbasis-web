...
$sql_ujian_exe = mysqli_query($sql_ujian);
$data_ujian = mysqli_fetch_assoc($sql_ujian_exe);
$nilai_exe = mysqli_query("select nilai,detail_jawaban from nilai where id_user='".$_SESSION['nis_kj']."' and id_ujian='".$id_ujian."'");
$data_nilai = mysqli_fetch_assoc($nilai_exe);
...
$sql_pil_exe = mysqli_query($sql_pil);
if(mysqli_num_rows($sql_pil_exe) > 0){	
$data_pil_arr = array();
while($data = mysqli_fetch_assoc($sql_pil_exe)){
...
$sql_soal_exe = mysqli_query($sql_soal);
$no = 0;
while($soalnya = mysqli_fetch_assoc($sql_soal_exe)){
	$no++;
	echo '<div class="block-content collapse in">
			...
			//	$sql_pil_exe = mysqli_query($sql_pil);
			//	while($data_pil = mysqli_fetch_assoc($sql_pil_exe)){
