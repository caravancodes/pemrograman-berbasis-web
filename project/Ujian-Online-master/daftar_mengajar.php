...
$nama_kelas = mysqli_result(mysqli_query("select nama_kelas from kelas where id_kelas='".$id_kelas."'"),0);
?>
<div class="row-fluid">
	<div class="block">
...
$sql_mengajar_exe = mysqli_query($sql_mengajar);
$data_ajar = array();
while($data = mysqli_fetch_assoc($sql_mengajar_exe)){
	$data_ajar[$data['id_mp']] = $data['id_guru'];
	}
// buat daftar pengajar sebagai drop down
$sql_guru = "select * from guru";
$sql_guru_exe = mysqli_query($sql_guru);
$data_guru = array();
while($data = mysqli_fetch_assoc($sql_guru_exe)){
...
$sql_mapel_exe = mysqli_query($sql_mapel);
$no = 1;
while($data = mysqli_fetch_assoc($sql_mapel_exe)){
	echo "<tbody>
			<tr>
				<td>".$no++."</td>
				<td>".$data['nama_mp']."</td>
